...
def get_db():
    if 'db' not in g:
        ...
        g.cursor = g.db.cursor() # set the cursor attribute in g
    return g.db, g.cursor # return the db and cursor objects
...
def create_user_table():
    ...
    ''')
    conn.commit()
# create a table for storing product data if it does not exist already
...
def check_password(username, password, hashed_password):
    return bcrypt.checkpw(password.encode('utf-8'), hashed_password.encode('utf-8')) and username == hashed_password.split(b'$')[1]

@app.before_request
def before_request():
    g.db, g.cursor = get_db() # set the db and cursor attributes in g
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if not re.match(r'[^@]+@[^@]+\.[^@]+', email):
            ...
            g.cursor.execute('INSERT INTO users (username, email, password) VALUES (%s, %s, %s)', (username, email, hashed_password))
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        g.cursor.execute('SELECT * FROM users WHERE username = %s', (username.decode(),))
        user = g.cursor.fetchone()
        if user and check_password(username, password, user[3].decode()):
